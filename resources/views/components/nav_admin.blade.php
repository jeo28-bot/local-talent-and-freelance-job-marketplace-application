...
<div class="flex fixed w-full pointer-events-none">
    ...
    <div id="vert_nav" class="bg-[#1E2939] w-auto min-h-[100vh] flex flex-col items-center border-r-1 border-gray-600 max-lg:fixed max-lg:w-80 max-sm:w-65 z-50 pointer-events-auto ">
        ...
        <div class="p_font text-white flex flex-col w-full px-5 gap-2 max-sm:px-3">
            ...
            {{-- link 6 --}}
            <a href="{{route('admin.announcements')}}" id="nav_applications" class="flex items-center gap-2 py-2 hover:bg-gray-700 px-5 rounded-lg max-sm:text-sm {{ request()->routeIs('admin.announcements') ? 'admin_selected_nav' : '' }}">
            ...
            {{-- link 9 --}}
             <a href="{{ route('admin.history_log') }}" id="nav_applications" class="flex items-center gap-2 py-2 hover:bg-gray-700 px-5 rounded-lg max-sm:text-sm {{ request()->routeIs('admin.history_log') ? 'admin_selected_nav' : '' }}">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true" data-slot="icon" class="size-6 max-sm:size-5">
