...
@section('content')
        ...
        <div class="pt-35 pl-85 max-[1270px]:pl-75 max-lg:pl-7  pr-10 max-sm:pr-5 pb-15">
            ...
            <div class="sm:w-xl lg:w-2xl mx-auto px-5 max-sm:px-3 mb-10">
                <div class="flex items-center justify-between flex-wrap">
                    ...
                    <a href="{{ route('admin.public_profile', ['name' => $user->name]) }}"
                ...
                @if ($status === 'online')
                    {{-- ✅ ONLINE STATUS --}}
                    <p class="p-2 rounded-lg text-center home_p_font mb-4 bg-green-200 text-green-600! border border-green-400 flex items-center justify-center gap-2 max-sm:text-sm"> 
                ...
                @elseif ($status === 'offline')
                    {{-- ⚫ OFFLINE STATUS --}}
                    <p class="p-2 rounded-lg text-center home_p_font mb-4 bg-gray-200 text-gray-600! border border-gray-400 flex items-center justify-center gap-2 max-sm:text-sm"> 
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-5">
                ...
                @elseif ($status === 'suspended')
                    {{-- 🚫 SUSPENDED STATUS --}}
                    <p class="p-2 rounded-lg text-center home_p_font mb-4 bg-red-200 text-red-600! border border-red-400 flex items-center justify-center gap-2 max-sm:text-sm"> 
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-5">
                            <path stroke-linecap="round" stroke-linejoin="round" d="m9.75 9.75 4.5 4.5m0-4.5-4.5 4.5M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
                        </svg>
                        {{ ucfirst($status) }}
                ...
                @else
                    ...
                    <p class="p-2 rounded-lg text-center home_p_font mb-4 bg-yellow-200 text-yellow-600! border border-yellow-400 flex items-center justify-center gap-2 max-sm:text-sm">
                        Unknown Status
                    </p>
                @endif
                        ...
                        <div class="p-4 bg-gray-300 rounded-lg shadow-sm mb-4 flex flex-col gap-3">
                            ...
                            <div class="p_font flex items-center bg-white px-4 py-2 rounded-xl gap-2 shadow-sm cursor-pointer hover:bg-gray-200">
                                ...
                                <button class="open-delete-modal cursor-pointer" 
                                        data-url="{{ route('admin.uploads.destroy', ['name' => $user->name, 'id' => $file->id]) }}">
    ...
    {{-- modal section --}}
            ...
            <div id="delete_job_warning" class="hidden modal_bg min-h-screen fixed top-0 z-40 w-full flex items-center justify-center px-5">
                <div class="px-5 py-3 bg-white rounded-xl -mt-20">
                    <h2 class="text-xl sub_title_font font-semibold mb-2">Delete User?</h2>
                    <p class="home_p_font text-gray-600 mb-5">This action cannot be undone. <br>Are you sure you want to delete this user?</p>
                    ...
                    <div class="flex gap-2">
                        ...
                        {{-- Confirm delete form --}}
                        <form id="confirm_delete_file_form" action="{{ route('admin.delete_user', ['name' => $user->name]) }}" method="POST">
                            @csrf
                            @method('DELETE')
            ...
            <div id="ceredential_uploads_modal" class="modal_bg min-h-screen fixed top-0 z-40 w-full flex items-center justify-center px-5 hidden">
                <div class="w-lg px-5 py-3 bg-gray-200 rounded-xl -mt-10">
                    ...
                    <div class="p-3 bg-white rounded-lg shadow-sm">
                        <form action="{{ route('admin.upload_files', ['name' => $user->name]) }}" method="POST" enctype="multipart/form-data">
                            @csrf
            ...
            <div id="logout_warning_modal" class="modal_bg min-h-screen fixed top-0 z-40 w-full flex items-center justify-center hidden max-sm:px-5">
                <div class="px-5 py-3 bg-white rounded-xl w-sm max-sm:w-full">
                    ...
                    <div class="flex gap-2 max-sm:flex-wrap">
                        <button id="cancel_logout"
                            class="bg-[#1e2939] cursor-pointer sub_title_font text-blue-400 px-4 py-2 rounded-lg hover:bg-[#374151] max-sm:text-sm">
                            Cancel
                        </button>

                        {{-- Suspend form --}}
                        <form action="{{ route('admin.update_profile', ['name' => $user->name]) }}" method="POST">
                            @csrf
                            @method('PUT')
            ...
            <div class="edit_user_details_modal modal_bg w-full min-h-screen fixed top-0 z-40 flex items-center justify-center px-5 hidden" id="edit_skill_modal">
                <div class="bg-gray-200 rounded-xl p-3 w-96 max-sm:w-80">
                    ...
                    <form action="{{ route('admin.update_profile', ['name' => $user->name]) }}" method="POST" class="p-3 bg-white rounded-lg shadow-sm">
                        @csrf
                        @method('PUT')
                        {{-- input 1 --}}
                        <div class="input_control">
                            <label for="username" class="p_font">Username: <span class="text-red-400">*</span></label>
                            <input type="text" name="username" id="username" placeholder="Enter your new username" class="p-2 shadow-sm w-full rounded-lg border-2 border-gray-400 mb-2" value="{{ $user->name }}" required>
                        </div>
                          {{-- input 2 --}}
                        <div class="input_control">
                            <label for="email" class="p_font">Email: <span class="text-red-400">*</span></label>
                            <input type="email" name="email" id="email" placeholder="Enter your new email" class="p-2 shadow-sm w-full rounded-lg border-2 border-gray-400 mb-2" value="{{ $user->email }}" required>
                        </div>
                          {{-- input 3 --}}
                        <div class="input_control">
                            <label for="phoneNum" class="p_font">Phone #: <span class="text-red-400">*</span></label>
                            <input type="number" name="phoneNum" id="phoneNum" placeholder="Enter your new phone #" class="p-2 shadow-sm w-full rounded-lg border-2 border-gray-400 mb-2" value="{{ $user->phoneNum }}" required>
                        </div>
                          {{-- input 4 --}}
                        <div class="input_control mb-2">
                            <label for="address" class="p_font">Address <span class="text-red-400">*</span></label>
                            <input type="text" name="address" id="address" placeholder="Enter your new address" class="p-2 shadow-sm w-full rounded-lg border-2 border-gray-400 mb-2" value="{{ $user->address }}" required>
                        </div>
                        @if(session('success'))
            ...
            <div class="edit_skills_modal modal_bg w-full min-h-screen fixed top-0 z-40 flex items-center justify-center px-5 hidden" id="edit_skill_modal">
                <div class="bg-gray-200 rounded-xl p-5 w-96 max-sm:w-80">
                    ...
                    <form action="{{ route('admin.update_skills', ['name' => $user->name]) }}" method="POST" id="skill_form" >
                        @csrf
                ...
                <div class="w-sm px-5 py-3 bg-gray-200 rounded-xl flex items-center flex-col">
                    ...
                    <div class="p-2 bg-white flex flex-col items-center rounded-lg shadow-sm mb-2 pb-5">
                        ...
                        Use JPG or PNG under 2MB for best quality.</p>
                            <form action="{{ route('admin.update_profile_picture', ['name' => $user->name]) }}" method="POST" enctype="multipart/form-data" class="mt-3 p_font flex flex-col items-center gap-2">
                                @csrf
            ...
            <div class="edit_user_about_modal modal_bg w-full min-h-screen fixed top-0 z-40 flex items-center justify-center px-5 hidden" id="edit_user_about_modal">
                <div class="bg-gray-200 rounded-xl p-3 w-lg max-sm:w-lg">
                   ...
                   <form action="{{ route('admin.update_about', ['name' => $user->name]) }}" method="POST" class="px-3 py-2 bg-white rounded-lg shadow-sm">
                        @csrf

                        <div class="input_control">
                            <label for="about_details" class="home_p_font mb-2 ">About Details:</label>
                            <textarea
                                name="about_details"
                                id="about_details"
                                placeholder="Write something about yourself here..."
                                class="p-2 shadow-sm w-full rounded-lg border-2 border-gray-400 mb-2 h-32">{{ old('about_details', $user->about_details) }}</textarea>
                        </div>
    ...
    <script src="{{ asset('js/admin/edit_profile.js') }}"></script>
