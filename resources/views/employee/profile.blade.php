    ...
    @include('components.nav_employee')
     ...
     <section class="w-full min-h-[80vh] px-10 py-10 max-sm:py-5 max-sm:px-4 ">
        <div class="sm:w-xl lg:w-2xl mx-auto px-5 max-sm:px-3 mb-10">
            ...
            <p class="home_p_font mb-3 max-sm:mb-2 text-sm flex items-center gap-2">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
            ...
            {{-- working status --}}
            <div class="p_font mb-2">
                @if($isWorking)
                    <span class="p_font px-3 py-1 bg-green-200 text-green-800 rounded-lg shadow-sm text-sm max-sm:px-1 max-sm:text-xs">Currently Working</span>
                @else
                    <span class="p_font px-3 py-1 bg-red-200 text-red-800 rounded-lg shadow-sm text-sm max-sm:px-1 max-sm:text-xs">Not currently working</span>
                @endif
            </div>
            ...
            <div class="flex mb-3 items-center">
                <a href="{{ route('employee.public_profile', ['name' => Auth::user()->name]) }}" class="text-blue-500 hover:underline p_font text-sm">
                    View public profile
                </a>
