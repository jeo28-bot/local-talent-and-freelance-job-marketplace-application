...
@section('content')
    ...
    <section class="w-full flex min-h-[80vh] flex-col items-center  px-20 max-lg:px-10 max-sm:px-5 pt-10 max-lg:pt-5">
        <div class="xl:w-6xl  mx-auto px-5 max-sm:px-2 mb-10 w-full max-lg:px-0">
            <div class="flex items-center justify-between max-lg:flex-col max-lg:gap-2 max-xl:items-start max-xl:mb-4 mb-5">
                <div>
                    <h1 class="sub_title sm:text-xl">Archived Applied</h1>
                    <p class="home_p_font text-sm">Manage and review your archived job applied.</p>
                </div>
                ...
                <div class="ml-auto max-lg:w-sm max-sm:w-full">
                    ...
                    {{-- search inputs --}}
                    <form action="{{ route('employee.arch_applied') }}" method="GET" class="bg-white  shadow-sm rounded-lg w-sm max-lg:w-full p_font max-sm:text-sm flex items-center">
            ...
            <div class="w-full h-[300px] flex flex-col justify-center items-center border-2 border-dashed border-gray-400 rounded-lg mt-1">
                ...
                <p class="p_font text-gray-500 text-center p-5 max-sm:text-sm">No archived applications yet. Check back later!</p>
            </div>
            @else
              ...
              {{-- table div --}}
                        ...
                        <tbody>
                            ...
                            <tr  class="applicant_row border-b-2 border-gray-300 py-2 hover:bg-gray-200 ">
                                <td class="px-4 py-2 p_font max-lg:text-sm home_p_font">
                                    {{ $loop->iteration + ($archivedApplications->currentPage() - 1) * $archivedApplications->perPage() }}
                                </td>
                                <td class="px-4 py-2 p_font max-lg:text-sm capitalize">    
                                    {{ '2025'.$application->id }}
                                </td>
                                <td class="px-4 py-2 p_font max-lg:text-sm">
                                <a href="{{ route('employee.public_profile', $application->user->name) }}" class="underline text-blue-700 hover:text-blue-400">
                                ...
                                <td class="px-4 py-2 p_font text-sm 
                                 ...
                                 <td class="px-4 py-2 p_font max-lg:text-sm capitalize flex max-xl:flex-col">
                                    <form action="{{ route('employee.restore_archived_applications', $application->id) }}" method="POST">
                                        @csrf
                                        @method('PUT')
                                ...
                                <td class="px-4 py-2 p_font max-lg:text-sm">
                                    <div class="flex gap-1">
                                            ...
                                            data-jobtitle="{{ $application->job ? e($application->job->job_title) : 'N/A' }}"
                                            ...
                                            <form action="{{ route('employee.force_delete_archived_applications', $application->id) }}" method="POST"      class="delete-applicant-form"
                                            >
                                                @csrf
                                                @method('DELETE')
                ...
                {{-- Custom Pagination --}}
                @if ($archivedApplications->total() > 10)
                    <div id="posting_pagination" class="w-full mx-auto flex items-center max-sm:flex-col max-sm:items-center gap-2">
                        <h3 class="home_p_font text-sm max-sm:text-xs">
                            {{ $archivedApplications->firstItem() ?? 0 }} to {{ $archivedApplications->lastItem() ?? 0 }} of {{ $archivedApplications->total() ?? 0 }} results
                        </h3>

                        <div class="flex ml-auto gap-2 max-sm:ml-0">
                            {{-- Previous button --}}
                            @if ($archivedApplications->onFirstPage())
                            ...
                            @else
                                <a href="{{ $archivedApplications->previousPageUrl() }}" class="job_posting_button bg-[#1E2939] text-white px-5 py-2 max-sm:py-2 max-sm:px-5 rounded-lg text-sm max-sm:text-xs">Previous</a>
                            @endif

                            {{-- Next button --}}
                            @if ($archivedApplications->hasMorePages())
                                <a href="{{ $archivedApplications->nextPageUrl() }}" class="job_posting_button bg-[#1E2939] text-white px-5 py-2 max-sm:py-2 max-sm:px-5 rounded-lg hover:opacity-90 text-sm max-sm:text-xs">Next</a>
                            @else
             ...
             <div class="w-full bg-white p-3 rounded-lg shadow-sm ">
                <div class="input_control flex flex-col mb-1">
                    <label for="fullName" class="home_p_font text-black! max-sm:text-sm">
                        Job title: <span id="applicant_job_title" class="font-semibold!">—</span>
                    </label>
                </div>
                <div class="input_control flex flex-col mb-1">
                    <label for="fullName" class="home_p_font text-black! max-sm:text-sm">
    ...
    <div id="delete_job_warning" class="hidden modal_bg min-h-screen fixed top-0 z-40 w-full flex items-center justify-center px-5">
        <div class="px-5 py-3 bg-white rounded-xl -mt-20">
            <h2 class="text-xl sub_title_font font-semibold mb-2">Delete Application?</h2>
            <p class="home_p_font text-gray-600 mb-5">This action cannot be undone. <br>Are you sure you want to delete this application?</p>
