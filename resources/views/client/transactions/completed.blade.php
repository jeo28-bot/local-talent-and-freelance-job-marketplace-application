...
@section('content')
        ...
        <div class="xl:w-6xl mx-auto px-5 max-sm:px-3 mb-10 pb-10">
            ...
            <div  class="flex items-center justify-between mb-3">
                ...
                <div class="flex justify-end gap-2">
                     <button id="openTotalSpendingBtn" class="p_font bg-[#1e2939] text-green-400 px-3 py-2 rounded-lg hover:opacity-80 max-lg:text-sm! max-sm:px-2 max-sm:py-1.5 flex items-center gap-1 cursor-pointer">
                    ...
                    <a href="{{route('client.arch_transactions')}}" class="p_font bg-[#1e2939] text-blue-400 px-3 py-2 rounded-lg hover:opacity-80 max-lg:text-sm! max-sm:px-2 max-sm:py-1.5 flex items-center gap-2">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true" data-slot="icon" class="size-6 max-lg:size-5">
    ...
    {{-- modal and script section --}}
    {{-- view total spending modal --}}
    <div id="totalSpending" class="hidden modal_bg min-h-screen fixed top-0 z-40 w-full flex items-center justify-center px-5">
        <div class="px-5 py-3 bg-white rounded-xl -mt-20 w-lg max-sm:w-full shadow-sm">
            ...
            <p class="p_font text-sm text-gray-500 font-light!">
                This is the total amount you have spent on completed transactions. It includes all payments made to employees for their work on your posted jobs. You can review the details of each transaction in the completed transactions section.
            </p>

           
            <div class="flex gap-2 justify-end">
                <button id="closetotalSpendingBtn" type="button"
    ...
    {{-- view total spending modal JS --}}
    <script>
        const totalSpendingModal = document.getElementById('totalSpending');
        const openTotalSpendingBtn = document.getElementById('openTotalSpendingBtn');
        const closeTotalSpendingBtn = document.getElementById('closetotalSpendingBtn');

        // Open modal
        openTotalSpendingBtn.addEventListener('click', () => {
            totalSpendingModal.classList.remove('hidden');
            document.body.classList.add('overflow-hidden');
        });

        // Close modal (button)
        closeTotalSpendingBtn.addEventListener('click', () => {
            closeTotalSpendingModal();
        });

        // Close modal when clicking outside
        totalSpendingModal.addEventListener('click', (e) => {
            if (e.target === totalSpendingModal) {
                closeTotalSpendingModal();
            }
        });
        ...
        document.addEventListener('keydown', (e) => {
            if (e.key === 'Escape' && !totalSpendingModal.classList.contains('hidden')) {
                closeTotalSpendingModal();
            }
        });

        function closeTotalSpendingModal() {
            totalSpendingModal.classList.add('hidden');
