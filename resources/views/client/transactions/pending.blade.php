...
@section('content')
        ...
        <div class="xl:w-6xl mx-auto px-5 max-sm:px-3 mb-10 pb-10">
            <div class="flex items-center justify-between max-lg:flex-col max-lg:gap-2 max-xl:items-start max-xl:mb-4 mb-5">
                ...
                <div class="ml-auto max-lg:w-sm max-sm:w-full">
                    ...
                    {{-- search inputs --}}
                    <form action="{{ route('client.transactions.pending') }}" method="GET" class="bg-white shadow-sm rounded-lg max-sm:w-full w-sm p_font max-sm:text-sm flex items-center">
                ...
                <div class="bg-white rounded-lg shadow-sm mb-5">
                    ...
                    <div class="w-full bg-[#1e2939] rounded-t-lg px-4 py-3 flex items-start justify-between">
                        <div>
                            ...
                            <br>
                            <a href="{{ route('client.public_profile', $transaction->employee->name) }}" class="home_p_font sm:text-lg text-gray-400 cursor-pointer hover:underline">
                                {{ $transaction->employee->name ?? 'Unknown Client' }}
                            </a>
                        </div>
                    ...
                    <div class="px-4 py-3">
                        <div class="flex justify-between items-start">
                            ...
                            {{-- options button --}}
                            <a href="{{ route('client.chat', $transaction->employee->name) }}" class="p-1.5 rounded-full border border-gray-500 hover:bg-gray-200 max-sm:p-1">
                        ...
                        <div class="w-full flex justify-end">
                            @if ($transaction->status === 'requested')
                                    <button type="submit" class="openPayNowBtn bg-[#1e2939] cursor-pointer sub_title_font text-blue-400 px-4 py-2 rounded-lg hover:bg-[#374151] max-sm:text-sm"
                                    data-transaction-id="{{ $transaction->id }}"
                                    data-job-title="{{ $transaction->job_title }}"
                                    data-employee-name="{{ $transaction->employee->name ?? 'Unknown Employee' }}"
                                    data-amount="₱{{ number_format($transaction->amount, 2) }}"
                                    data-payment-method="{{ ucfirst($transaction->payment_method) }}"
    ...
    {{-- paynow modal --}}
    <div id="payNowModal" class="report_modal fixed top-0 left-0 w-full h-full z-50 max-sm:px-6 bg-black/50 flex justify-center items-start overflow-y-auto py-10 hidden">
        <div class="sm:w-lg mt-20 mx-auto p-5 bg-gray-200 rounded-xl shadow-sm">
            <div class="flex justify-between items-center mb-2 gap-5">
                <h3 class="sub_title_font max-sm:text-sm">Employee payment details:</h3>
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" id="closePayNowModalBtn" class="size-5 max-sm:size-4 cursor-pointer hover:bg-red-400 rounded-sm bg-gray-300">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M6 18 18 6M6 6l12 12" />
                </svg>
            </div>

            <form id="payNowForm" action="" method="POST">
                @csrf
                ...
                <div class="w-full bg-white p-3 rounded-lg shadow-sm mb-4">
                    ...
                    <input 
                            id="transaction_ref_no" 
                            name="transaction_ref_no"
                            type="text" 
                            class="max-sm:text-sm p-2.5 rounded-lg border border-gray-300 max-sm:p-1.5 w-full mb-2" 
                ...
                <div class="flex">
                    <button id="" type="submit" class="cursor-pointer p_font bg-[#1E2939] text-white px-7 max-sm:px-5 py-3 rounded-lg hover:opacity-90 ml-auto max-sm:text-sm">
                       Paid
                    </button>
                </div>
            </form>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const closeBtn = document.getElementById('closePayNowModalBtn');
    ...
    <script>
        document.getElementById('transaction_ref_no').addEventListener('input', function (e) {
            let value = e.target.value.toUpperCase().trim();

            // allow partial "N", "NA", "N/" or full "N/A"
            if (/^N\/?A?$/.test(value)) {
